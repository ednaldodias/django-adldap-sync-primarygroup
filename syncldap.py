...
class Command(BaseCommand):
    ...
    def sync_ldap_users(self, ldap_users):
        ...
        for cname, attributes in ldap_users:
                ...
                for name, attribute in attributes.items():
                    try:
                        if ((name.lower() == 'thumbnailphoto') or (name.lower() == 'jpegphoto') or (name.lower() == 'thumbnaillogo')):
                            ...
                            if (name.lower() == 'objectsid'):                                                           ###!!!!
                                defaults[self.conf_LDAP_SYNC_USER_ATTRIBUTES[name]] = self.sid_to_str(attribute[0])     ###!!!!
                            else:                                                                                       ###!!!!
            ...
            if 'userAccountControl' not in attributes:                                                      ###!!!!
                logger.debug("ATENÇÃO =" + username + " Sem 'userAccountControl'")                          ###!!!!
                print("ATENÇÃO =" + username + " Sem 'userAccountControl'")                                 ###!!!!
                attributes['userAccountControl'] =  [b'512']                                                ###!!!!

            user_account_control = int(attributes[self.ATTRIBUTE_DISABLED][0].decode('utf-8'))
            user_primaryGroupId = (attributes['primaryGroupID'][0].decode('utf-8'))                         ###!!!
            primaryGroupSid = self.sid_to_str(attributes['objectSid'][0])                                   ###!!!
            primaryGroupSid = primaryGroupSid[:(primaryGroupSid.rindex("-") + 1)] + user_primaryGroupId     ###!!!

            user_is_disabled = (user_account_control and ((user_account_control & self.FLAG_UF_ACCOUNT_DISABLE) == self.FLAG_UF_ACCOUNT_DISABLE))
            if user_is_disabled:
                print(username + " Desabilitado")
                try:
            ...
            try:
                ...
                if (self.conf_LDAP_SYNC_GROUP_MEMBERSHIP):
                    membership_uri, ldap_membership =                   \
                        self.get_ldap_user_membership(                  \
                             attributes[self.conf_LDAP_SYNC_GROUP_MEMBERSHIP_DN_FIELD][0].decode('utf-8'),  \
                                 primaryGroupSid)                                                                     #####!!!!!
                    if ldap_membership:
    ...
    def get_ldap_user_membership(self, user_dn, primaryGroupSid):                               ####!!!!!
        ...
        objectsid_filter =                                                                 \
                '(&(objectClass=group)(member:1.2.840.113556.1.4.1941:=' + groups1[0][0] + '))'     ####!!!
        uri1, groups1 =                                                                     \
                self.ldap_search(objectsid_filter, self.conf_LDAP_SYNC_GROUP_ATTRIBUTES.keys(), \
                False, objectsid_filter)                                                            ####!!!
